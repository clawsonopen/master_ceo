...
Implementation status update (2026-04-07):

1. Phase 1 completed for dev runtime: single-instance guard now prevents duplicate `dev` startup for the same repo/port.
2. Phase 2 completed for dev runtime: startup preflight auto-recovery now cleans stale embedded-postgres/process state and retries safely.
3. Phase 3 completed for dev runtime: startup is health-gated and is not treated as ready until `/api/health` is healthy.
4. Remaining release work is productized UX for packaged apps (in-app repair action, startup state UX, and diagnostics export).
